...
describe('modelPricing (local models)', () => {
  it('exports metadata for default local models', () => {
    expect(LOCAL_MODELS['gemma4:26b']).toBeDefined();
    expect(LOCAL_MODELS['gemma4:26b'].label).toBe('Gemma 4 (27B MoE)');
  });
  ...
  it('getModelLabel returns known model label', () => {
    expect(getModelLabel('gemma4:26b')).toBe('Gemma 4 (27B MoE)');
  });
